...
class SaleModel extends AbstractModel {
    ...
    public function get(int $saleId): Sale {
        ...
        $query = <<<SQL 
SELECT b.id, b.title, b.author, b.price, sb.amount as stock, b.isbn FROM sale s LEFT JOIN sale_book sb ON s.id = sb.sale_id LEFT JOIN book b ON sb.book_id
        = b.id WHERE s.id = :id 
SQL;
        $sth = $this->db->prepare($query);
        $sth->execute(['id' => $saleId]);
    ...
    public function create(Sale $sale) {
        ...
        $query = <<<SQL 
INSERT INTO sale(customer_id, date) VALUES(:id, NOW()) 
SQL; 
        $sth = $this->db->prepare($query);
        ...
        $query = <<<SQL 
INSERT INTO sale_book(sale_id, book_id, amount) VALUES(:sale, :book, :amount) 
SQL;
        $sth = $this->db->prepare($query);
